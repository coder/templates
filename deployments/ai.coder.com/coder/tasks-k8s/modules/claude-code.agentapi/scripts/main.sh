...
WORKDIR="$ARG_WORKDIR"
SOURCE_SCRIPT="$ARG_SOURCE_SCRIPT"
...
if [ -n "${SOURCE_SCRIPT}" ]; then
    ...
    echo -n "${SOURCE_SCRIPT}" >"$module_path/source_script.sh"
    chmod +x "$module_path/source_script.sh"
    source "$module_path/source_script.sh"
fi
