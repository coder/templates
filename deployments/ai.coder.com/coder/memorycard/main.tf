...
locals {
  ...
  work_folder    = join("/", [local.home_folder, "memory-card-ai-demo"])
  workspace_name = random_uuid.this.result
  port           = data.coder_parameter.preview_port.value == "" ? 4200 : data.coder_parameter.preview_port.value
