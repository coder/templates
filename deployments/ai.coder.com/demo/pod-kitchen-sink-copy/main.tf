...
resource "coder_metadata" "workspace_info" {
  ...
  item {
    key   = "Pod Name"
    value = "coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}"
  }
}

data "coder_parameter" "repo_dropdown" {
  name = "repo_dropdown"
  display_name = "Select your repository"
  description = "All single-select parameters with options may now use the `form_type=\"dropdown\"` attribute for better organization."
  type = "string"

  form_type = "dropdown"

  dynamic "option" {
    for_each = concat([for i in range(0, 1024) : "repo-${i}"], [for i in range(0, 1024) : "2repo-${i}"])
    content {
      name  = "https://github.com/coder/${option.value}"
      value = "https://github.com/coder/${option.value}"
    }
  }
}