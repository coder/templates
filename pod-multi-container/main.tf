...
locals {
  ...
  golang-image = "coderintegration.jfrog.io/docker/coder/coder-demo/coder-demo-golang:latest"
...
resource "kubernetes_persistent_volume_claim" "pgadmin-directory" {
  metadata {
    name      = "pgadmin-coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}"
    namespace = var.workspaces_namespace
  }
  wait_until_bound = false
  spec {
...
resource "kubernetes_persistent_volume_claim" "postgres-data-directory" {
  metadata {
    name      = "postgres-data-coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}"
    namespace = var.workspaces_namespace
  }
  wait_until_bound = false
  spec {
...
resource "kubernetes_persistent_volume_claim" "dbeaver-directory" {
  metadata {
    name      = "dbeaver-coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}"
    namespace = var.workspaces_namespace
  }
  wait_until_bound = false
  spec {
